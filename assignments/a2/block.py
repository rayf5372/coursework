...
def _block_to_squares(
    board: Block,
) -> list[tuple[tuple[int, int, int], tuple[int, int], int]]:
    """Return a list of tuples describing all the total_squares that
     must be drawn in order to render this Block.
    ...
    if len(board.children) == 0:
        ...
        for block in board.children:
            total_squares.extend(_block_to_squares(block))
...
class Block:
    ...
    def swap(self, direction: int) -> bool:
        ...
        if len(self.children) != 0:
            if direction == SWAP_VERT:
                self.children[0], self.children[2] = (
                    self.children[2],
                    self.children[0],
                )
    ...
    def combine(self) -> bool:
        ...
        highest_count = 0
        tie = False
        for colour, colour_count in count.items():
            if colour_count > highest_count:
                new_colour = colour
                highest_count = colour_count
                tie = False
            elif colour_count == highest_count:
                tie = True

        if not tie and highest_count > 1:
    ...
    def create_copy(self) -> Block:
        ...
        """
        copy = Block(
            self.position, self.size, self.colour, self.level, self.max_depth
        )
        copy.children = []
        for child in self.children:
