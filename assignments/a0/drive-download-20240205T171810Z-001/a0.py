...
def all_within_grid(coords: list[tuple[int, int]], n: int) -> bool:
    ...
    >>> all_within_grid([(2, 3), (0, 1), (1, 0)], 4)
    True
    >>> all_within_grid([(5, 5), (2, 2), (3, 3)], 4)
    False
    """
    for coord in coords:
...
def reflect_vertically(coord: tuple[int, int], n: int) -> tuple[int, int]:
    ...
    (4, 2)
    >>> reflect_vertically((1, 1), 4)
    (2, 1)
    """
    return n - 1 - coord[0], coord[1]


@check_contracts
def reflect_points(
    line: list[tuple[int, int]], n: int
) -> list[tuple[int, int]]:
    """
    ...
    >>> reflect_points([(1, 0), (1, 1), (1, 2), (1, 3), (1, 4), (1, 5)], 6)
...
def is_column(squares: list[Square]) -> bool:
    ...
    False
    """
    base_row, base_col = squares[0].coord
    for square in squares[1:]:
        if square.coord[1] != base_col or square.coord[0] - base_row != 1:
            return False
...
def _is_diagonal(squares: list[Square], up: bool) -> bool:
    ...
    for square in squares[1:]:
        if up:
            if (square.coord[1] - cur_col != 1) or (
                square.coord[0] - cur_row != 1
            ):
                return False
        else:
            if (square.coord[1] - cur_col != 1) or (
...
class Line:
    ...
    Representation Invariants:
        - len(self) >= 4
        - is_row(self) or is_column(self) or is_diagonal(self)
    ...
    def has_fiar(self, coord: tuple[int, int]) -> bool:
        ...
        list_of_symbols = []
        fiar = False
        symbol = self[self._coord_to_location[coord]].symbol
        for square in self:
            if square.symbol == symbol and square.symbol is not None:
                list_of_symbols.append(square.coord)
            else:
                list_of_symbols = []

            if len(list_of_symbols) == 4 and coord in list_of_symbols:
                fiar = True
        return fiar
...
def create_rows_and_columns(
    ...
    for i in range(len(squares)):
        rows.append(Line(squares[i]))
        column = []
        for j in range(len(squares[0])):
            column.append(squares[j][i])
...
def create_mapping(
    ...
    for row in range(len(squares)):
        for column in range(len(squares)):
            coord = (row, column)
            mapping[coord] = [rows[row], columns[column]]
            mapping[coord].extend(diagonals_helper(coord, diagonals))
    return mapping


@check_contracts
def diagonals_helper(
    coord: tuple[int, int], diagonals: list[Line]
) -> list[Line]:
    """
    Helper function for create_mapping
    Returns a list of all diagonals that coord appear in.
    """
    result = []
...
def get_down_diagonal_starts(n: int) -> list[tuple[int, int]]:
    ...
    for column in range(1, n):
        results.append((0, column))
    valid_results = []
    for coord in results:
        if coord[0] < (n - 3) and coord[1] < (n - 3):
            valid_results.append(coord)
...
def get_all_down_diagonals(n: int) -> list[list[tuple[int, int]]]:
    ...
    for start_coord in starts:
        diagonal = get_down_diagonal(start_coord, n)
        all_down_diagonals.append(diagonal)
...
def get_coords_of_diagonals(n: int) -> list[list[tuple[int, int]]]:
    ...
    up = []
    for diagonals in down:
        up.append(reflect_points(diagonals, n))
...
def all_diagonals(squares: list[list[Square]]) -> list[Line]:
    ...
    for coords in diagonal_coords:
        line = []
        for coord in coords:
            line.append(Square(coord))
        diagonals.append(Line(line))
    return diagonals
